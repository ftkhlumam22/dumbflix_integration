...
import (
	...
	"net/http"
	"strconv"
	filmsdto "testdumpflix/dto/film"
...
func (h *handlerForFilm) GetFilm(w http.ResponseWriter, r *http.Request) {
	...
	filmResponse := filmsdto.FilmResponse{
		ID:            film.ID,
		Title:         film.Title,
		ThumbnailFilm: "http://localhost:5000/uploads/" + film.ThumbnailFilm,
		Description:   film.Description,
		...
		Episode: mapEpisodeToDto(film.Episode),
	}
...
func mapEpisodeToDto(episodeModels []models.Episode) (mapEpisodeResponse []filmsdto.EpisodeFilmResponse) {
	for _, episodeModel := range episodeModels {
		episodeResponse := filmsdto.EpisodeFilmResponse{
			ID:               episodeModel.ID,
			TitleEpisode:     episodeModel.TitleEpisode,
			ThumbnailEpisode: "http://localhost:5000/uploads/" + episodeModel.ThumbnailEpisode,
			LinkFilm:         episodeModel.LinkFilm,
			FilmID:           episodeModel.FilmID,
		}
		mapEpisodeResponse = append(mapEpisodeResponse, episodeResponse)
	}
	return
}
